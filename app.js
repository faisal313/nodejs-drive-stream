...
function prioritizeStreaming(torrent) {
    ...
    // Prioritize first 10 MB of the video file to enable quick start
    const highPrioritySize = Math.min(file.length, 10 * 1024 * 1024); // 10 MB

    for (let i = 0; i < highPrioritySize / file._torrent.pieceLength; i++) {
        file._torrent.select(i * file._torrent.pieceLength, (i + 1) * file._torrent.pieceLength, true, true);
    }
}
